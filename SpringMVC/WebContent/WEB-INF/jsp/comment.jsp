...
window.onload = function(){
	...
	$("#pay").click(function(){
		$.ajax({
			type:"POST",
			dataType:"text",
			data:"orderID="+$("#orderID").val()+"&productID="+$("#productID").attr("value")+"&commentcontent="+$("#comment").attr("value")+"&rank="+$("#rank").attr("value"),
			url:path+"/doComment",
...
<body>
 ...
 <input type="hidden" value="${commentcontent}" id="commentcontent"/>
<jsp:include page="/jsp/top"   />
<div class="border_top_cart">

      <label>请输入你的评论：</label> <input type="text" value="" id="comment" />
      <label>请输入你的评分：</label> <input type="text" value="" id="rank" />
      <hr/>
      <button id="pay"> 确认评论</button>
</div>
